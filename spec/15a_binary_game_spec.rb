...
describe BinaryGame do
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        expect(game_input.verify_input(1, 10, 2)).to eql(2)
      end
    end

    context 'when given invalid input as argument' do
      it 'returns nil' do
        expect(game_input.verify_input(1, 10, 100)).to eql(nil)
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    it 'increases guess_count by one' do
      max = game_display.instance_variable_get(:@guess_count)

      expect { game_display.display_turn_order(search_display) }.to change { game_display.instance_variable_get(:@guess_count) }.to(max + 1)
    end
